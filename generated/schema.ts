...
export class Bootstrap extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Bootstrap entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Bootstrap must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("Bootstrap", id.toString(), this);
    }
  }

  static loadInBlock(id: string): Bootstrap | null {
    return changetype<Bootstrap | null>(store.get_in_block("Bootstrap", id));
  }

  static load(id: string): Bootstrap | null {
    return changetype<Bootstrap | null>(store.get("Bootstrap", id));
  }
